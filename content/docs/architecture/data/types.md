...
W3DS data is IRI-grounded: every entity type, property, and value type is identified by an IRI and resolves to a published declaration with structural commitments and a description. That grounding is what makes the data legible across services and over time. The architecture commits to (a) IRI identity for vocabulary elements, (b) structural validation at write time against declared ranges, multiplicities, and constraints, and (c) alignment with explicit kinds (exact / close / broad / narrow / related) as the cross-vocabulary equivalence mechanism. It deliberately does *not* include RDFS/OWL reasoning machinery: subclass entailment, property chains, restriction satisfaction, and similar inference rules live in consumer logic, not in the eVault. Consumers are free to layer reasoning over the IRIs they choose to honor; the eVault treats IRIs as opaque addresses for declared shapes.
